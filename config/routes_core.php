			...
			foreach ($this->__matches as $rule)
			{
				...
				if (preg_match($rule['regex'], $request, $match))
				{
					...
					if (!isset($option['controller']) || $option['controller'] == '')
						throw new NoAvailableControllerException(isset($option['controller']) ? $option['controller'] : '');
...
echo $_GET['a'];
		...
		else
		{
			if (!@file_exists(dirname(__FILE__) . '/../app/controllers/' . $option['controller'] . '_controller.php'))
				throw new NoAvailableControllerException($option['controller']);
			...
			$temp = new \ActionController\TemporaryCollection();
